...
it("throws an error if a config file does not exist", async () => {
    const path = resolve(TEST_DATA_DIR, "does-not-exist.mjs");
    await expect(loadBuildConfig(path)).rejects.toThrow(/does not exist/);
});

it("throws an error if the config file does not have a default export", async () => {
    const path = resolve(TEST_DATA_DIR, "build-config-without-export.mjs");
    await expect(loadBuildConfig(path)).rejects.toThrow(/must provide a default export/);
});
